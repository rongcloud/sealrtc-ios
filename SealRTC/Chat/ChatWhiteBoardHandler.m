...
- (WhiteBoardView *)whiteBoardView
{
    if (!_whiteBoardView) {
        ...
        _whiteBoardView.frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight);
...
- (void)createMenuItemButtons
{
    ...
    CGFloat buttonInterval = (ScreenWidth - 20 - iconWidth *6) / 5.f;
    ...
    for (NSInteger i = 0; i < 6; i++)
    {
        ...
        {
            case 0: //画笔
                buttonName = NSLocalizedString(@"chat_white_pencil", nil);
                imageName = @"chat_white_pencil";
                break;
            case 1: //文字
                buttonName = NSLocalizedString(@"chat_white_text", nil);
                imageName = @"chat_white_text";
                break;
            case 2: //橡皮擦
                buttonName = NSLocalizedString(@"chat_white_eraser", nil);
                imageName = @"chat_white_eraser";
                break;
            case 3: //清空
                buttonName = NSLocalizedString(@"chat_white_clean", nil);
                imageName = @"chat_white_clean";
                break;
            case 4: //新建
                buttonName = NSLocalizedString(@"chat_white_new", nil);
                imageName = @"chat_white_create";
                break;
            case 5: //删除
                buttonName = NSLocalizedString(@"chat_white_delete", nil);
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"chat_white_join_failed", nil) message:@"" preferredStyle:UIAlertControllerStyleAlert];
